...
from sensors import calculate_mar
...
def run_calibration(cap, face_mesh, eye_det_cls):
    ...
    if calib_ears:
        ...
        baseline_mar = float(np.mean(calib_mars))
        mar_thresh   = baseline_mar + 0.45

        # [수정] nan 경고 방지 - calib_ears 있을 때만 평균 계산
        avg_str    = f"{baseline:.3f}"
        mar_str    = f"{baseline_mar:.3f}"
    else:
        baseline   = 0.28
        ear_thresh = 0.21
        baseline_mar = 0.15
    ...
    print(f"[INFO] 캘리브레이션 완료 | "
          f"EAR 평균: {avg_str} | EAR 임계값: {ear_thresh:.3f} | "
          f"MAR 평균: {mar_str} | MAR 임계값: {mar_thresh:.3f}")
    
    return ear_thresh, baseline, mar_thresh, baseline_mar
...
def main():
    ...
    # ── 캘리브레이션 ────────────────────────────────────────────
    ear_thresh, baseline, mar_thresh, baseline_mar = run_calibration(cap, face_mesh, None)
