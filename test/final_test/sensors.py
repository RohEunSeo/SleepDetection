...
class EyeDetector:
    ...
    def get_Gaze_Score(self, landmarks, frame_size):
        ...
        for lms_idx, iris_idx in [
            ...
            # 화면 내 절대 x 위치 대신 눈 크기를 기준으로 정규화해 좌우 비대칭을 줄인다.
            eye_width = max(xs.max() - xs.min(), 1e-6)
            score = LA.norm(iris - eye_center) / (eye_width * 5.0)
            scores.append(score)
...
def draw_mouth_box(frame, lms_raw, frame_size, yawn_detected, mar, mar_thresh):
    fw, fh = frame_size
